...
export default function EditCouponPage() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="mb-6 font-heading text-2xl font-bold">Edit Coupon</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        {error && <div className="rounded-lg bg-red-50 p-3 text-sm text-red-600">{error}</div>}
        <div><label className="block text-sm font-medium mb-1">Coupon Name</label>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div><label className="block text-sm font-medium mb-1">Usage Limit</label>
          ...
          <div><label className="block text-sm font-medium mb-1">Per User</label>
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving} className="px-6 py-2 bg-primary-600 text-white rounded-lg font-medium hover:bg-primary-700 disabled:opacity-50">{saving ? "Saving..." : "Save Changes"}</button>
          <button type="button" onClick={() => router.push("/admin/coupons")} className="px-6 py-2 border rounded-lg text-sm hover:bg-neutral-50">Cancel</button>
        </div>
      </form>
    </div>
  );
